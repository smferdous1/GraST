...
int main(int argC, char *argV[]) {
  ...
  if(resFile.empty() == false) {
    ...
    if(fexist == false) 
      fWrite<<"graph,n,m,algorithm,read,process,weight,card,mem,feat"<<std::endl;
    writeResults(getLastPartOfFilepath(fname),"grdymatch",stat,feat);
  }
  return 0;
}
